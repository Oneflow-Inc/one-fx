'''
Modified from https://github.com/pytorch/examples/blob/main/fx/primitive_library.py
'''
...
def sigmoid_lowp(x : oneflow.Tensor):
    x = x.float()
    x = x.sigmoid()
    return x.half()
...
onefx.wrap(sigmoid_lowp)

def add_lowp(a : oneflow.Tensor, b : oneflow.Tensor):
    a, b = a.float(), b.float()
    c = a + b
    return c.half()
...
traced = onefx.symbolic_trace(Foo())
print(traced.code)
"""
def forward(self, x, y):
...
def inline_lowp_func(n : onefx.Node):
    ...
    if n.op == 'call_function' and n.target.__module__ == inline_lowp_func.__module__:
        ...
        tracer = onefx.proxy.GraphAppendingTracer(n.graph)
        with n.graph.inserting_before(n):
            ...
            proxy_args = onefx.node.map_arg(n.args, lambda x: onefx.Proxy(x, tracer))
            proxy_kwargs = onefx.node.map_arg(n.kwargs, lambda x: onefx.Proxy(x, tracer))
...
class InliningTracer(onefx.Tracer):
    FNS_TO_INLINE = [add_lowp]

    def create_node(self, kind, target, args, kwargs, name=None, type_expr=None):
        if kind == 'call_function' and target in self.FNS_TO_INLINE:
            tracer = onefx.proxy.GraphAppendingTracer(self.graph)
            # Trace through the implementation of the function rather than
            # create a node
            proxy_args = onefx.node.map_arg(args, lambda x: onefx.Proxy(x, tracer))
            proxy_kwargs = onefx.node.map_arg(kwargs, lambda x: onefx.Proxy(x, tracer))
...
module = onefx.GraphModule(f, graph)
print(module.code)
"""
def forward(self, x, y):
    sigmoid_lowp = __main___sigmoid_lowp(x);  x = None

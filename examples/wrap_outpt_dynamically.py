...
import numpy as np
import sys
sys.path.append('../one-fx')
from onefx import GraphModule, Node, Proxy, symbolic_trace, wrap, global_wrap, global_wrap_oneflow_all
...
activation_functions = {
    ActivationFunction.RELU: oneflow.nn.ReLU(),
    ActivationFunction.LEAKY_RELU: oneflow.nn.LeakyReLU(),
    ActivationFunction.PRELU: oneflow.nn.PReLU(),
}
...
# Example call
x, y = oneflow.randn(5, 3), oneflow.randn(5, 3)
...
assert np.allclose(new_output.numpy(), oneflow.nn.LeakyReLU()(orig_output).numpy())
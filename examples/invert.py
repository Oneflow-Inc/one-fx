'''
Modified from https://github.com/pytorch/examples/blob/main/fx/invert.py
'''
...
def add_inverse(a, b):
    ...
    invert_mapping[b] = a
inverses = [
    (oneflow.sin, oneflow.arcsin),
    (oneflow.cos, oneflow.arccos),
    (oneflow.tan, oneflow.arctan),
    (oneflow.exp, oneflow.log),
]
for a, b in inverses:
    add_inverse(a, b)
...
# one output.
def invert(model: oneflow.nn.Module) -> oneflow.nn.Module:
...
def f(x):
    return oneflow.exp(oneflow.tan(x))

res = invert(f)
print(res.code)
"""
def forward(self, output):
    log_1 = oneflow.log(output);  output = None
    arctan_1 = oneflow.arctan(log_1);  log_1 = None
    return arctan_1
"""
print(f(res((oneflow.arange(5) + 1))))  # [1., 2., 3., 4, 5.]
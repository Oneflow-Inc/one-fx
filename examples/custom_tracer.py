'''
Modified from https://github.com/pytorch/examples/blob/main/fx/custom_tracer.py
'''
...
from onefx import symbolic_trace, Tracer, Graph, GraphModule, Node
...
of `onefx.symbolic_trace`--can be subclassed to override various
...
refer to the docstrings of the methods in the Tracer class.
...
`assert` to the more FX-friendly `oneflow.assert`.
...
"""
Custom Tracer #1: Trace Through All `oneflow.nn.ReLU` Submodules
...
is called a "leaf module". By default, all modules in the Pyoneflow
standard library (`oneflow.nn`) are leaf modules. We can change this
by creating a custom Tracer and overriding `is_leaf_module`. In this
case, we'll keep the default behavior for all `oneflow.nn` Modules except
for `ReLU`.
"""
...
class LowerReluTracer(Tracer):
    def is_leaf_module(self, m : oneflow.nn.Module, qualname : str):
        if isinstance(m, oneflow.nn.ReLU):
            return False

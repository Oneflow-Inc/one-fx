...
import onefx.utils._pytree as pytree
...
def _is_from_oneflow(obj: Any) -> bool:
    ...
    if module_name is not None:
        ...
        return (
            base_module == 'oneflow' and
            not module_name.startswith("oneflow._dynamo.") and
            not module_name.startswith("oneflow._inductor.")
        )

    name = getattr(obj, '__name__', None)
    # exclude oneflow because oneflow.oneflow.oneflow.oneflow works. idk mang
...
class CodeGen(object):
    ...
    def _gen_python_code(self, nodes, root_module: str, namespace: _Namespace, *, verbose: bool = False) -> PythonCode:
        ...
        def add_global(name_hint: str, obj: Any):
            ...
            """
            if _is_from_oneflow(obj) and obj != oneflow.device:  # to support registering oneflow.device
        ...
        def stringify_shape(shape : oneflow.Size) -> str:
        ...
        def emit_node(node : Node):
            ...
            if verbose:
                ...
                from oneflow._subclasses.fake_tensor import FakeTensor
                from onefx.experimental.proxy_tensor import py_sym_types
            ...
            if node.op == 'placeholder':
                ...
                # not currently supported in normal FX graphs, but generated by oneflowdynamo
        ...
        if len(wrapped_fns) > 0:
            wrap_name = add_global('wrap', onefx.wrap)
...
class Graph:
    ...
    .. code-block:: text
        ...
        graph(x):
            ...
            %sum_1 : [#users=1] = call_function[target=oneflow.sum](args = (%relu_1,), kwargs = {dim: -1})
            %topk_1 : [#users=1] = call_function[target=oneflow.topk](args = (%sum_1, 3), kwargs = {})
            return topk_1
    ...
    def placeholder(self, name: str, type_expr: Optional[Any] = None,
        ...
        Args:
            ...
            type_expr (Optional[Any]): an optional type annotation representing the
                ...
                subsequently in oneflowScript compilation).
    ...
    def get_attr(self, qualified_name: str, type_expr: Optional[Any] = None) -> Node:
        ...
        .. note::
            ...
            as ``Graph.create_node``.
        """
        def _get_attr_reference_exists(mod: oneflow.nn.Module, qualified_name: str) -> bool:
            module_path, _, name = qualified_name.rpartition(".")

            try:
                submod: oneflow.nn.Module = mod.get_submodule(module_path)
            ...
            if (not isinstance(res, oneflow.nn.Module)
            ...
            for node in self.nodes:
                if node.op == 'call_function':
                    if not callable(node.target):
                        raise ValueError(f'Node {node} target {node.target} has type {oneflow.typename(node.target)} but '
    ...
    def on_generate_code(
        ...
        .. code-block:: python
            ...
            gm: onefx.GraphModule = ...

            # This is a code transformer we want to register. This code
            # transformer prepends a pdb import and trace statement at the very
            # beginning of the generated oneflow.fx code to allow for manual

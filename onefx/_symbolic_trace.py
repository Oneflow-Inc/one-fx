...
# from oneflow._C import ScriptObject  # type: ignore[attr-defined]
...
def is_oneflow_wrapped_function_and_try_get(name):
    possible_real_name = name.split('.')[-1]
    for package in _oneflow_default_wrapped_packages:
        if hasattr(package, name):
            return True, getattr(package, name)
...
class Tracer(TracerBase):
    ...
    def __init__(
        ...
        Args:
            ...
            autowrap_modules (Tuple[ModuleType]): defaults to `(math, )`,
                ...
                without needing to use onefx.wrap(). Backward-compatibility for
                this parameter is guaranteed.

            autowrap_function (Tuple[Callable, ...]): defaults to `()`,
                Python functions that should be wrapped automatically without
                needing to use onefx.wrap(). Backward compabilibility for this
    ...
    def is_leaf_module(self, m: oneflow.nn.Module, module_qualified_name: str) -> bool:
        ...
        """
        return (
            (m.__module__.startswith("oneflow.nn") or m.__module__.startswith("oneflow.ao.nn"))
            and not isinstance(m, oneflow.nn.Sequential)
        )
    ...
    def getattr(self, attr: str, attr_val: Any, parameter_proxy_cache: Dict[str, Any]):
        ...
        if self.proxy_buffer_attributes and isinstance(attr_val, oneflow.Tensor):
...
class _Patcher(object):
    ...
    def patch(
        ...
        if name not in frame_dict and (hasattr(builtins, name) or is_oneflow_wrapped_function_and_try_get(name)[0]):
...
def symbolic_trace(
    ...
    `b` to trace through this.
        ...
        f = onefx.symbolic_trace(f, concrete_args={'b': False})
        assert f(3, False)  == 6
    ...
    overspecializing, pass in `onefx.PH` for values that shouldn't be
    specialized. For example::
        ...
        def f(x):
            ...
            for v in x.values():
                out += v
            return out
        f = onefx.symbolic_trace(f, concrete_args={'x': {'a': onefx.PH, 'b': onefx.PH, 'c': onefx.PH}})

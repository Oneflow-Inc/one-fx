"""
Modified from https://github.com/pytorch/pytorch/blob/master/torch/fx/node.py
"""
...
import types
import warnings
# from onefx.operator_schemas import normalize_function, normalize_module, ArgsKwargsPairs
...
def _get_qualified_name(func: Callable[..., Any]) -> str:
    ...
    module = module.replace('oneflow._ops', 'oneflow.ops')  # WAR for bug in how oneflow.ops assigns module
...
class Node:
    ...
    def __init__(self, graph: 'Graph', name: str, op: str, target: 'Target',
        ...
        if op == 'call_function':
            if not callable(target):
                raise ValueError(f'Node [graph = {graph}, name = \'{name}\'] target {target} has type {oneflow.typename(target)} '
        ...
        # being invoked, e.g add, layer1, or oneflow.add
    ...
    def args(self, a : Tuple[Argument, ...]):
        ...
        depends on the node's opcode. See the ``onefx.Graph`` docstring for more
        information.
        """
    ...
    def _pretty_print_target(self, target):
        ...
        3) other callables will be printed with qualfied name, e.g. oneflow.add
        """
    ...
    #         self, root : oneflow.nn.Module, arg_types : Optional[Tuple[Any]] = None,
    ...
    #     Args:
    #         root (oneflow.nn.Module): Module upon which to resolve module targets.
...
def map_arg(a: Argument, fn: Callable[[Node], Argument]) -> Argument:
    ...
    """
    assert callable(fn), "oneflow.onefx.map_arg(a, fn): fn must be a callable"

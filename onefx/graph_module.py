...
# and then tools like oneflowScript will be able to get source info.
...
def _deserialize_graph_module(forward, body: Dict[Any, Any]) -> oneflow.nn.Module:
    """
...
def _assign_attr(from_obj: Any, to_module: oneflow.nn.Module, target: str):
    *prefix, field = target.split('.')
    ...
    # If it is a tensor and not a parameter attribute of a module, it should be a named buffer.
    # So, we register it as a named buffer in the target module.
    if isinstance(from_obj, oneflow.Tensor) and not isinstance(from_obj, oneflow.nn.Parameter):
...
class GraphModule(oneflow.nn.Module):
    """
    GraphModule is an nn.Module generated from an onefx.Graph. Graphmodule has a
...
"""
        ...
        for buffer_name, buffer in self._buffers.items():
            if buffer is None:
                continue
            model_str += f"{tab*2}self.register_buffer('{buffer_name}', oneflow.empty({list(buffer.shape)}, dtype={buffer.dtype}))\n"

        for param_name, param in self._parameters.items():
            if param is None:
                continue
            model_str += f"{tab*2}self.{param_name} = oneflow.nn.Parameter(oneflow.empty({list(param.shape)}, dtype={param.dtype}))\n"

        model_str += f"{tab*2}self.load_state_dict(oneflow.load(r'{folder}/state_dict.pt'))\n"
    ...
    # Passing Tracer as argument allows subclasses extending onefx.GraphModule
    # define their own Tracer (extending onefx.Tracer).
...
#             return t.__oneflow_function__(patched_cat, (), args, kwargs)

...
def replace_pattern(
    ...
    .. code-block:: python
        ...
        import oneflow
        from oneflow.fx import symbolic_trace, subgraph_rewriter
        ...
        class M(oneflow.nn.Module):
            ...
            def forward(self, x, w1, w2):
                m1 = oneflow.cat([w1, w2]).sum()
                m2 = oneflow.cat([w1, w2]).sum()
                return x + oneflow.max(m1) + oneflow.max(m2)

        def pattern(w1, w2):
            return oneflow.cat([w1, w2]).sum()
    ...
    ``p = oneflow.cat([a, b])`` in ``pattern``, you could match
    ``m = oneflow.cat([a, b])`` in the original ``forward`` function,
    ...
    .. code-block:: python
        ...
        def pattern(x, y):
            return oneflow.neg(x) + oneflow.relu(y)
    ...
    .. code-block:: python
        ...
        def forward(self, x, w1, w2):
            stack_1 = oneflow.stack([w1, w2])
            sum_1 = stack_1.sum()
            stack_2 = oneflow.stack([w1, w2])
            sum_2 = stack_2.sum()
            max_1 = oneflow.max(sum_1)
            add_1 = x + max_1
            max_2 = oneflow.max(sum_2)
            add_2 = add_1 + max_2
            return add_2
    """
...
def _replace_pattern(
    ...
    from onefx.passes.utils.matcher_utils import SubgraphMatcher, InternalMatch
    ...
    if isinstance(replacement, oneflow.nn.Module):

...
class Partition:
    def __init__(self, name: str):
        ...
        self.graph: onefx.graph.Graph = onefx.graph.Graph()
        self.environment: Dict[onefx.node.Node, onefx.node.Node] = {}
...
def split_module(
    m: GraphModule,
    root_m: oneflow.nn.Module,
    split_callback: Callable[[onefx.node.Node], int],
    ...
    Args:
        ...
        root_m (oneflow.nn.Module): root nn module. Not currently used. Included
            because the root nn module is usually transformed via
            onefx._symbolic_trace.symbolic_trace (see example below)
        split_callback (Callable[[onefx.node.Node], int]): Callable function
    ...
    Example:
        ...
        This is a sample setup:
            ...
            import oneflow
            from onefx.symbolic_trace import symbolic_trace
            from onefx.graph_module import GraphModule
            from onefx.node import Node
            from onefx.passes.split_module import split_module
    ...
    orig_nodes: Dict[str, onefx.node.Node] = {}

    def record_cross_partition_use(
        def_node: onefx.node.Node, use_node: Optional[onefx.node.Node]
    ):  # noqa: B950
    ...
    for node in m.graph.nodes:
        if hasattr(node, "_fx_partition"):
            ...
            gathered_args = onefx.graph.map_arg(node.args, lambda n: environment[n])
            gathered_kwargs = onefx.graph.map_arg(
    ...
    base_mod_env: Dict[str, onefx.node.Node] = {}
    base_mod_graph: onefx.graph.Graph = onefx.graph.Graph()
    base_mod_attrs: Dict[str, onefx.graph_module.GraphModule] = {}
    for node in m.graph.nodes:
    ...
    for partition_name in construct_order_partitions:
        ...
        base_mod_attrs[partition.submod_name] = onefx.graph_module.GraphModule(
        ...
        if len(partition.outputs) > 1:
            ...
            output_val_proxy = onefx.proxy.Proxy(output_val)
    ...
    for node in m.graph.nodes:
        if node.op == "output":
            base_mod_graph.output(
                onefx.graph.map_arg(node.args[0], lambda n: base_mod_env[n.name])
            )  # noqa: B950

    return onefx.graph_module.GraphModule(base_mod_attrs, base_mod_graph)

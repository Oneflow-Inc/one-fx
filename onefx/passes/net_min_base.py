"""
Modified from https://github.com/pytorch/pytorch/blob/master/torch/fx/passes/net_min_base.py
"""
...
class _MinimizerBase:
    ...
    def __init__(
        ...
        assert isinstance(module, onefx.GraphModule)
    ...
    def run_a(self, mod: onefx.GraphModule, inputs: Tensors) -> TensorOrTensors:
        """
    ...
    def run_b(self, mod: onefx.GraphModule, inputs: Tensors) -> TensorOrTensors:
        """
        Run `mod` with `inputs` and generate output. The output will be compared with
        output of run_a().
        """
    ...
    def _store_outputs(
        ...
        # Only one output
        if isinstance(output_node.args[0], onefx.Node):
    ...
    def _get_submod_inputs(
        self, main_module: onefx.GraphModule, submod_path: str
    ) -> Tuple[Tensors, Tensors]:
        """
        Try get submodule inputs from stored outputs. If not found then use
        oneflow_glow.get_submod_inputs to get the inputs.
        ...
        # outputs as inputs. Otherwise, use `oneflow_glow.get_submod_inputs`
        # to get the inputs.
    ...
    def _build_submodule(self, nodes: NodeSet) -> Tuple[onefx.GraphModule, str]:
        """
        ...
        Returns:
            split_module (onefx.GraphModule): the module after split.
    ...
    def _run_and_compare(
        self, split_module: onefx.GraphModule, submod_name: str, output_names: Names
    ):
        """

...
NodeList = List[onefx.Node]
NodeSet = Set[onefx.Node]
Names = List[str]
...
def get_acc_ops_name(k):
    if isinstance(k, str):
        ...
        module = k.__module__.replace('oneflow._ops', 'oneflow.ops')  # WAR for bug in how oneflow.ops assigns module
        return f"{module if module else ''}.{k.__name__}"


@compatibility(is_backward_compatible=False)
def get_node_target(submodules: Mapping[str, oneflow.nn.Module], node: onefx.Node) -> str:
    """
...
@compatibility(is_backward_compatible=False)
def is_node_output_tensor(node: onefx.Node) -> bool:
    ...
    return type_ is not None and issubclass(type_, oneflow.Tensor)
...
class FxNetAccFusionsFinder:
    ...
    def __init__(self, module: onefx.GraphModule, acc_nodes: NodeSet):
        self.module = module
    ...
    def __call__(self) -> Dict[onefx.Node, NodeSet]:
        result: Dict[onefx.Node, NodeSet] = {}
...
@compatibility(is_backward_compatible=False)
def legalize_graph(gm: onefx.GraphModule) -> onefx.GraphModule:
    """
    ...
    for node in gm.graph.nodes:
        if indeg[node] == 0:
            queue.append(node)
    env: Dict[onefx.Node, onefx.Node] = {}

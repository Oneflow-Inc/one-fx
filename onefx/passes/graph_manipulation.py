"""
Modified from https://github.com/pytorch/pytorch/blob/master/torch/fx/passes/graph_manipulation.py
"""
...
from onefx.node import (
    map_arg,
    Node,
    Target,
)
from onefx.passes.shape_prop import ShapeProp
...
def get_size_of_all_nodes(
    fx_module: GraphModule, args: Optional[List[oneflow.Tensor]] = None
) -> None:
...
def get_size_of_node(fx_module: GraphModule, node: Node) -> size_bytes:
    ...
    if tensor_meta.is_quantized:
        size_per_elem_bytes = oneflow._empty_affine_quantized(
            [], dtype=tensor_meta.dtype
        ).element_size()
    else:
        size_per_elem_bytes = oneflow.tensor([], dtype=tensor_meta.dtype).element_size()

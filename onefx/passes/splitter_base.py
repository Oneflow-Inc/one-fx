...
class SplitResult(NamedTuple):
    ...
    split_module: onefx.GraphModule
...
class _SplitterBase:
    ...
    def find_deps(self) -> Dict[onefx.Node, NodeSet]:
        """
        ...
        """
        deps: Dict[onefx.Node, NodeSet] = defaultdict(set)
        for node in self.module.graph.nodes:
            if node.op not in CALLABLE_NODE_OPS:
                continue
    ...
    def _lower_model_to_backend(
        self, mod: onefx.GraphModule, inputs: Tensors
    ) -> oneflow.nn.Module:
        """
    ...
    def _draw_graph_based_on_node_support(
        self, mod: onefx.GraphModule, supported_nodes: NodeList
    ):
        color_map = {
            "default": "AliceBlue",
    ...
    def node_support_preview(self, dump_graph: bool = False):
        ...
        for node in self.module.graph.nodes:
            ...
            arg_dtypes = [
                get_dtype(arg) if isinstance(arg, onefx.Node) else None
    ...
    def find_reverse_deps(
        ...
        """
        result: Dict[onefx.Node, NodeSet] = defaultdict(set)
    ...
    def update_reverse_deps_for_fusions(
        self, deps: Dict[onefx.Node, NodeSet]
    ):
        processed_node = set()
    ...
    def split(self, remove_tag: bool = False) -> onefx.GraphModule:
    ...
    def __call__(self) -> onefx.GraphModule:
        subgraphs = self.put_nodes_into_subgraphs()
        subgraphs = self.remove_small_acc_subgraphs(subgraphs)

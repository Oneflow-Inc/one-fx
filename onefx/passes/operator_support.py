...
# onefx.Node.target typename, as returned by `get_node_target()`
TargetTypeName = str

# Arguments' dtypes for a given node, see `OperatorSupport`
SupportedArgumentDTypes = t.Optional[
    t.Tuple[
        t.Sequence[t.Sequence[oneflow.dtype]],
        t.Dict[str, t.Sequence[oneflow.dtype]],
    ]
]
...
class OperatorSupportBase(abc.ABC):
    """Interface for determining if a onefx.Node is supported by a backend"""
    @abc.abstractmethod
    def is_node_supported(
        self, submodules: t.Mapping[str, oneflow.nn.Module], node: onefx.Node
    ) -> bool:
...
class OperatorSupport(OperatorSupportBase):
    ...
    e.g. (None, [oneflow.float]) indicates that we don't care about the type of
    ...
    def is_node_supported(
        ...
        for i, dtypes in enumerate(args_dtypes):
            ...
            if not isinstance(node.args[i], onefx.Node):
                continue
        ...
        for k, dtypes in kwargs_dtypes.items():
            ...
            # If arg is not a node then we don't check it
            if not isinstance(node.kwargs[k], onefx.Node):
                continue
...
IsNodeSupported = t.Callable[[t.Mapping[str, oneflow.nn.Module], onefx.Node], bool]
...
class OpSupports:
    ...
    """
    @classmethod
    def decline_if_input_dtype(cls, dtype: oneflow.dtype) -> OperatorSupportBase:
        """Report a node as non-supported, if any of its arguments is of dtype"""

        def _decline_if_input_dtype(
            submodules: t.Mapping[str, oneflow.nn.Module],
            node: onefx.Node,
        ) -> bool:
...
def _get_arg_dtype(arg: onefx.Node) -> t.Any:
    assert isinstance(arg, onefx.Node)

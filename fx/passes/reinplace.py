...
from fx.utils._pytree import tree_map, tree_flatten, tree_map_only
from fx.utils._storage_weak_ref import StorageWeakRef
...
def _is_view_op(tgt):
    if tgt is not None and isinstance(tgt, oneflow._ops.OpOverload):
        schema = tgt._schema
...
class _FunctionalizationMetadataProp(fx.Interpreter):
    ...
    def run_node(self, node: Node):
        ...
        node_args = node.args
        if node.target is oneflow.ops.aten.copy_.default:
        ...
        if node.op == 'call_function':
            ...
            #    %split_tensor : [#users=2] = call_function[target=oneflow.ops.aten.split.Tensor](args = (%x_1, 2), kwargs = {})
...
# TODO: we should also figure this info out using oneflowgen.
def _maybe_get_inplace_op(op):
    # __module__ seems broken; it returns oneflow._ops.aten which doesn't exist
    if not isinstance(op, oneflow._ops.OpOverload):
        return None
    ...
    maybe_namespace_module = getattr(oneflow.ops, op_namespace)
...
_VIEW_INVERSE_MAP = {
    # oneflow.ops.aten.diagonal_scatter.default: oneflow.ops.aten.diagonal.default,
    # oneflow.ops.aten.select_scatter.default: oneflow.ops.aten.select.int,
    # oneflow.ops.aten.slice_scatter.default: oneflow.ops.aten.slice.Tensor,
    # oneflow.ops.aten.as_strided_scatter.default: oneflow.ops.aten.as_strided.default,
}
    ...
    for t in tensor_aliases:
        ...
        for n in usage_nodes:
            ...
            if n in tensor_aliases:
                if isinstance(n.target, oneflow._ops.OpOverload) or n.target == _operator.getitem:
                    continue
...
def reinplace(gm, *sample_args):
    ...
    (1) Perform some initial checks on the metadata of "a" and "args..."
      ...
      (1a) Check that the self argument we're attempting to reinplace
           ...
           Similarly, we can't convert oneflow.ge(a, b) into a.ge_(b),
           ...
           e.g. oneflow.ones(1).expand(4, 4).add_(1)
    ...
    (2) Check that "a" and all of its outstanding aliases are not used anywhere
        ...
        Given this program:
        def f(x):
            a = oneflow.ops.aten.add(x, x)
            b = oneflow.ops.aten.diagonal(a)
            oneflow.ops.aten.fill_(b, 0)
            return d

        Functionalization will emit the following:
        def f(x):
            a = oneflow.ops.aten.add(x, x)
            b = oneflow.ops.aten.diagonal(a, 0, 1)
            b_updated = oneflow.ops.aten.fill(b, 0)
            a_updated = oneflow.ops.aten.diagonal_scatter(a, b_updated, 0, 1)
            return a_updated
    ...
    (3) Perform the actual re-inplacing on foo!
      ...
      (3a) {view}_scatter ops.
        ...
        Post functionalization, that will look like:
          a = oneflow.zeros(2)
          b = oneflow.ones(1)
          a_updated = oneflow.select_scatter(a, b, 0, 0)
        ...
        Before:
          a_updated = oneflow.select_scatter(a, b, args...)
        After:
    ...
    for idx, node in enumerate(gm.graph.nodes):
        if node.op == 'call_function':
            ...
            if not isinstance(node.target, oneflow._ops.OpOverload):
                continue
            if len(node.target._schema.arguments) < 1:
                continue
            if type(node.target._schema.arguments[0].type) != oneflow.TensorType:
                continue

            # Step 1a: Check that the self argument we're attempting to reinplace
            # has the same size/stride as the output.
            # For example, we shouldn't try to reinplace oneflow.add(scalar_tensor, larger_tensor)
            ...
            if len(self_flattened) == len(node_flattened):
                for self_meta, node_meta in zip(self_flattened, node_flattened):
                    ...
                    # e.g. oneflow.ones(1).expand(4, 4).add_(1)
                    if oneflow._debug_has_internal_overlap(self_meta) == 1:
                        self_has_wrong_metadata = True
            # Here, we (optimistically) assume that a.resize(b) is valid to re-inplace,
            # Since users should never really be calling the functional "oneflow.ops.aten.resize"
            # op directly in their programs.
            if self_has_wrong_metadata and node.target != oneflow.ops.aten.resize.default:
                continue
            ...
            # When we see an operator like `b = oneflow.slice_scatter(a, ...)`,
            ...
            if node.target in _VIEW_INVERSE_MAP and node not in all_later_view_inverse_nodes_to_delete:
                ...
                # Before:
                #   base_updated = oneflow.ops.aten.slice_scatter.default(base, mutated_slice, args...)
                # After:
                #   slice = oneflow.ops.aten.slice.default(base, args...)
                #   slice.copy_(mutated_slice)
                with gm.graph.inserting_before(node):
                    ...
                    copy_node = gm.graph.create_node(
                        'call_function', oneflow.ops.aten.copy_.default, (slice_node, mutated_slice_node,), {})

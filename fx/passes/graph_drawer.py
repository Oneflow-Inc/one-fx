...
from fx.node import _get_qualified_name, _format_arg
...
if HAS_PYDOT:
    ...
    class FxGraphDrawer:
        """
        Visualize a fx.Graph with graphviz
        Basic usage:
        ...
        def __init__(
            ...
            for node in graph_module.graph.nodes:
                ...
                if not isinstance(leaf_node, fx.GraphModule):
                    continue
        ...
        def _get_node_style(self, node: fx.Node) -> Dict[str, str]:
            template = {
                "shape": "record",
                "fillcolor": "#CAFFE3",
                "style": '"filled,rounded"',
        ...
        def _get_leaf_node(
            self, module: oneflow.nn.Module, node: fx.Node
        ) -> oneflow.nn.Module:
            py_obj = module
        ...
        def _typename(self, target: Any) -> str:
            if isinstance(target, oneflow.nn.Module):
                ret = oneflow.typename(target)
        ...
        def _stringify_tensor_meta(self, tm: TensorMetadata) -> str:
            ...
            if tm.is_quantized:
                ...
                if qscheme in {
                        oneflow.per_tensor_affine,
                        oneflow.per_tensor_symmetric,
                }:
                    result += "|" + "q_scale" + "=" + str(tm.qparams["scale"]) + r"\n"
                    result += "|" + "q_zero_point" + "=" + str(tm.qparams["zero_point"]) + r"\n"
                elif qscheme in {
                        oneflow.per_channel_affine,
                        oneflow.per_channel_symmetric,
                        oneflow.per_channel_affine_float_qparams,
                }:
        ...
        def _get_tensor_label(self, t: oneflow.Tensor) -> str:
        ...
        def _to_dot(
            ...
            for node in graph_module.graph.nodes:
                ...
                def get_module_params_or_buffers():
                    for pname, ptensor in chain(
                        ...
                        label1 = (
                            ...
                            if isinstance(ptensor, oneflow.nn.Parameter)
                            else "buffer" + r"\l"
                        )
                ...
                if node.op == "call_module":
                    ...
                    if not ignore_parameters_and_buffers and not isinstance(leaf_module, oneflow.fx.GraphModule):
    ...
    if not TYPE_CHECKING:
        @compatibility(is_backward_compatible=False)
        class FxGraphDrawer:
            def __init__(self, graph_module: fx.GraphModule, name: str, ignore_getattr: bool = False):

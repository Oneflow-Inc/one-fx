...
module_fetch_book: Dict[Type, Tuple[int, List[str], Callable[[str, int], str]]] = {
    oneflow.nn.modules.linear.Linear: (1, ["weight", "bias"], default_matching),
    oneflow.nn.modules.conv.Conv2d: (
        1, ["weight", "bias", "kernel_size", "stride", "padding", "dilation", "groups", "padding_mode"], default_matching
    ),
    oneflow.nn.modules.batchnorm.BatchNorm2d: (2, ["weight", "bias", "running_mean", "running_var", "eps"], default_matching),
    oneflow.nn.modules.pooling.AdaptiveAvgPool2d: (1, [], default_matching),
    oneflow.nn.modules.pooling.MaxPool2d: (
        1, ["kernel_size", "stride", "padding", "dilation", "return_indices", "ceil_mode"], default_matching
    ),
    oneflow.nn.modules.activation.ReLU: (1, ["inplace"], default_matching),
}
...
def extract_attrs_for_lowering(mod: nn.Module) -> Dict[str, Any]:
    ...
    attrs_for_lowering["name"] = oneflow.typename(mod)

    if type(mod) in module_fetch_book:
        version, param_to_fetch, matching_method = module_fetch_book[type(mod)]
        if version < mod._version:
            raise RuntimeError(f"Fetcher version {version} try to fetch {oneflow.typename(mod)} version {mod._version}, "
        ...
        raise RuntimeError(f"{oneflow.typename(mod)} is not in the module_fetch_book yet, "

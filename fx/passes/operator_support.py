...
class OperatorSupportBase(abc.ABC):
    ...
    def is_node_supported(
        self, submodules: t.Mapping[str, oneflow.nn.Module], node: fx.Node
    ) -> bool:
...
class OperatorSupport(OperatorSupportBase):
    ...
    def is_node_supported(
        ...
        for i, dtypes in enumerate(args_dtypes):
            ...
            if not isinstance(node.args[i], fx.Node):
                continue
        ...
        for k, dtypes in kwargs_dtypes.items():
            ...
            # If arg is not a node then we don't check it
            if not isinstance(node.kwargs[k], fx.Node):
                continue
...
IsNodeSupported = t.Callable[[t.Mapping[str, oneflow.nn.Module], fx.Node], bool]
...
def _get_arg_dtype(arg: fx.Node) -> t.Any:
    assert isinstance(arg, fx.Node)

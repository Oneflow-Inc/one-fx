...
import fx
from fx.graph import map_arg
from .tools_common import NodeList
from fx._compatibility import compatibility
from fx.passes.utils import lift_subgraph_as_module, HolderModule
...
class Component:
    ...
    getattr_maps: Dict[fx.Node, fx.Node] = field(default_factory=dict)
    constructor_args: List[str] = field(default_factory=list)
    gm: Optional[fx.GraphModule] = None


@compatibility(is_backward_compatible=False)
def split_by_tags(gm: fx.GraphModule, tags: List[str]) -> fx.GraphModule:
    """
    ...
    def flatten(x: fx.node.Argument) -> NodeList:
        """
    ...
    node_remapping: Dict[fx.Node, fx.Node] = {}

    # Mapping from node in original module or created submodules to
    # corresponding component.
    node_to_component: Dict[fx.Node, Component] = {}
    ...
    for comp in all_components:
        ...
        if len(outs) == 1:
            ...
            for i, o in enumerate(comp.orig_outputs):
                ...
                main_remapping[o] = fx.Proxy(main_node)[i].node  # type: ignore[index]
    ...
    return fx.GraphModule(main_root, main_g)

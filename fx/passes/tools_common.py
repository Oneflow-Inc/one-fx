...
import oneflow
import fx
from fx.node import _get_qualified_name
...
@compatibility(is_backward_compatible=False)
def get_node_target(submodules: Mapping[str, oneflow.nn.Module], node: fx.Node) -> str:
    """
...
@compatibility(is_backward_compatible=False)
def is_node_output_tensor(node: fx.Node) -> bool:
...
class FxNetAccFusionsFinder:
    ...
    def __init__(self, module: fx.GraphModule, acc_nodes: NodeSet):
        self.module = module
    ...
    def __call__(self) -> Dict[fx.Node, NodeSet]:
        result: Dict[fx.Node, NodeSet] = {}
...
@compatibility(is_backward_compatible=False)
def legalize_graph(gm: fx.GraphModule) -> fx.GraphModule:
    """

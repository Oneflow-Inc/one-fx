...
class Partition:
    def __init__(self, name: str):
        ...
        self.graph: fx.graph.Graph = fx.graph.Graph()
        self.environment: Dict[fx.node.Node, fx.node.Node] = {}
...
def split_module(
    ...
    split_callback: Callable[[fx.node.Node], int],
    ...
    Args:
        ...
        root_m (oneflow.nn.Module): root nn module. Not currently used. Included
            ...
            fx._symbolic_trace.symbolic_trace (see example below)
        split_callback (Callable[[fx.node.Node], int]): Callable function
    ...
    Example:
        ...
        This is a sample setup:
            ...
            from fx.passes.split_module import split_module
    ...
    orig_nodes: Dict[str, fx.node.Node] = {}

    def record_cross_partition_use(
        def_node: fx.node.Node, use_node: Optional[fx.node.Node]
    ):  # noqa: B950
    ...
    for node in m.graph.nodes:
        if hasattr(node, "_fx_partition"):
            ...
            gathered_args = fx.graph.map_arg(node.args, lambda n: environment[n])
            gathered_kwargs = fx.graph.map_arg(
    ...
    base_mod_env: Dict[str, fx.node.Node] = {}
    base_mod_graph: fx.graph.Graph = fx.graph.Graph()
    base_mod_attrs: Dict[str, fx.graph_module.GraphModule] = {}
    for node in m.graph.nodes:
    ...
    for partition_name in construct_order_partitions:
        ...
        base_mod_attrs[partition.submod_name] = fx.graph_module.GraphModule(
        ...
        if len(partition.outputs) > 1:
            ...
            output_val_proxy = fx.proxy.Proxy(output_val)
    ...
    for node in m.graph.nodes:
        if node.op == "output":
            base_mod_graph.output(
                fx.graph.map_arg(node.args[0], lambda n: base_mod_env[n.name])
            )  # noqa: B950

    return fx.graph_module.GraphModule(base_mod_attrs, base_mod_graph)

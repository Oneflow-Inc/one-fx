...
from fx.passes.operator_support import OperatorSupport
from fx.passes.tools_common import CALLABLE_NODE_OPS
from fx.passes.fake_tensor_prop import FakeTensorProp
from fx.utils._pytree import tree_map
...
class CudaGraphsSupport(OperatorSupport):
    ...
    def is_node_supported(self, submodules, node: fx.Node) -> bool:
        ...
        if node.target in [oneflow.ops.aten.embedding_dense_backward.default]:
            return False
        ...
        def find_not_cuda(t):
            ...
            if isinstance(t, oneflow.Tensor) and t.device.type != 'cuda':
                found_not_cuda = True

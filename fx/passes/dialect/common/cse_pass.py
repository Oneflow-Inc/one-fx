...
import oneflow
import fx
from fx.passes.infra.pass_base import PassBase, PassResult
from fx.utils._pytree import tree_flatten
...
@fx._compatibility.compatibility(is_backward_compatible=False)
...
class CSEPass(PassBase):
    ...
    def call(self, graph_module: GraphModule) -> PassResult:
        """
        Return a new copy of fx.GraphModule with CSE applied to the input graph

        Example usage:

        from fx.experimental.proxy_tensor import make_fx
        def f(a):
            b = a * a
            c = a * a
            return b+c

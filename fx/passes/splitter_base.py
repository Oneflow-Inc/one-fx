...
import oneflow
import fx
from fx.passes.graph_manipulation import get_size_of_node
...
class _SplitterBase:
    ...
    def find_deps(self) -> Dict[fx.Node, NodeSet]:
        """
        ...
        """
        deps: Dict[fx.Node, NodeSet] = defaultdict(set)
        for node in self.module.graph.nodes:
            if node.op not in CALLABLE_NODE_OPS:
                continue
    ...
    def _lower_model_to_backend(
        self, mod: fx.GraphModule, inputs: Tensors
    ) -> oneflow.nn.Module:
        """
    ...
    def _draw_graph_based_on_node_support(
        self, mod: fx.GraphModule, supported_nodes: NodeList
    ):
        color_map = {
            "default": "AliceBlue",
    ...
    def node_support_preview(self, dump_graph: bool = False):
        ...
        for node in self.module.graph.nodes:
            ...
            arg_dtypes = [
                get_dtype(arg) if isinstance(arg, fx.Node) else None
    ...
    def find_reverse_deps(
        ...
        """
        result: Dict[fx.Node, NodeSet] = defaultdict(set)
    ...
    def update_reverse_deps_for_fusions(
        self, deps: Dict[fx.Node, NodeSet]
    ):
        processed_node = set()
    ...
    def split(self, remove_tag: bool = False) -> fx.GraphModule:
    ...
    def __call__(self) -> fx.GraphModule:
        subgraphs = self.put_nodes_into_subgraphs()
        subgraphs = self.remove_small_acc_subgraphs(subgraphs)

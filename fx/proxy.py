...
import fx.traceback as fx_traceback
...
class TracerBase:
    ...
    def _find_user_frame(self):
        ...
        pt_files = ['fx/proxy.py',
                    'fx/_symbolic_trace.py',
                    'fx/experimental/proxy_tensor.py',
                    'oneflow/_ops.py',
                    'oneflow/_tensor.py',
                    'oneflow/utils/_python_dispatch.py',
                    'oneflow/_prims_common/wrappers.py',
                    'oneflow/_refs/__init__.py',
                    'oneflow/_refs/nn/functional/__init__.py'
                    ]
        while frame:
            frame = frame.f_back
...
class Proxy:
    ...
    (``oneflow`` function calls, method calls, operators) that they touch
    ...
    the "Proxy" section in `oneflow/fx/OVERVIEW.md`
    """
    ...
    @classmethod
    def __oneflow_function__(cls, orig_method, types, args=None, kwargs=None):
        ...
        oneflow.fx.node.map_aggregate(args, find_tracer)
        oneflow.fx.node.map_aggregate(kwargs, find_tracer)

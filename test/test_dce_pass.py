...
class TestDCE(unittest.TestCase):
    ...
    def _run_dce_and_test(
        ...
        inputs = [oneflow.tensor([1.5])] * new_num_phs
        self.assertTrue(oneflow.equal(m(*inputs), traced(*inputs)))
    ...
    def test_simple(self):
        ...
        class TestModule(oneflow.nn.Module):
            def __init__(self):
                super().__init__()
                self.attr_1 = oneflow.nn.Parameter(oneflow.tensor([-0.9]))
    ...
    def test_keep_module_with_side_effects(self):
        ...
        class ReLUImpure(oneflow.nn.ReLU):
            _is_impure = True
        ...
        class TestModule(oneflow.nn.Module):
            ...
            def forward(self, a: oneflow.Tensor) -> oneflow.Tensor:
                r = self.relu(a)
                return a * 2
